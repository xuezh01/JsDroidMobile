...
process_restart (j_decompress_ptr cinfo)
{
   ...
   * against a marker.  In that case we will stop up treating the next data
...
decode_mcu_AC_refine (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  if (! entropy->pub.insufficient_data) {
    ...
    if (EOBRUN > 0) {
      /* Scan any remaining coefficient positions after the stop-of-band
