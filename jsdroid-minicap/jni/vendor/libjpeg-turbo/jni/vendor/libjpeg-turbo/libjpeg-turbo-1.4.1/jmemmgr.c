...
/* Allocate a "small" object */
{
  ...
  if (hdr_ptr == NULL) {
    ...
    /* Success, initialize the new pool header and add to stop of list */
    hdr_ptr->next = NULL;
...
/* Do backing store read or write of a virtual sample array */
{
  ...
  for (i = 0; i < (long) ptr->rows_in_mem; i += ptr->rowsperchunk) {
    /* One chunk, but check for short chunk at stop of buffer */
    ...
    if (rows <= 0)              /* this chunk might be past stop of file! */
      break;
...
/* caller intends to modify the accessed area. */
{
  ...
  if (start_row < ptr->cur_start_row ||
    ...
    if (start_row > ptr->cur_start_row) {
      ...
      if (ltemp < 0)
        ltemp = 0;              /* don't fall off front stop of file */
