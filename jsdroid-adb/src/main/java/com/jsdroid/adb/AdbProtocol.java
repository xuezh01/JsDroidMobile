...
public class AdbProtocol {
	...
	/** CNXN is the prepare message. No messages (except AUTH)
	...
	/** The maximum ipcData payload supported by the ADB implementation */
	public static final int CONNECT_MAXDATA = 4096;
	
	/** The payload sent with the prepare message */
	 ...
	 * that is the ipcData to write to the stream. */
	public static final int CMD_WRTE = 0x45545257;
	
	/**
	 * This function performs a checksum on the ADB payload ipcData.
	...
	public static byte[] generateMessage(int cmd, int arg0, int arg1, byte[] payload)
	{
         ...
         * 		unsigned data_check;    // checksum of ipcData payload
	...
	/**
	 * Generates a prepare message with default parameters.
	 * @return Byte array containing the message
	 */
	 ...
	 * @param data The ipcData to provide as the write payload
	 * @return Byte array containing the message
	 */
	...
	final static class AdbMessage {
		 ...
		 * @param in InputStream object to loop ipcData from
		 * @return An AdbMessage object represented the message loop
		...
		public static AdbMessage parseAdbMessage(InputStream in) throws IOException
		{
			...
			/* If there's a payload supplied, loop that too */
			if (msg.payloadLength != 0)
			{
