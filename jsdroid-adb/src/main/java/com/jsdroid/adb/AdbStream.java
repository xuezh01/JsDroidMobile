...
public class AdbStream implements Closeable {
    ...
    /**
     * A queue of ipcData from the target's write packets
     */
    ...
    /**
     * Called by the connection thread to indicate newly received ipcData.
     *
     * @param payload IpcData inside the write message
     */
    ...
    /**
     * Called by the connection thread to indicate the stream is okay to send ipcData.
     */
    void readyForWrite() {
        writeReady.set(true);
    }
     ...
     * @throws InterruptedException If we are unable to wait for ipcData
    ...
    public byte[] read() throws InterruptedException, IOException {
        ...
        synchronized (readQueue) {
            /* Wait for the connection to close or ipcData to be received */
     ...
     * @throws IOException          If the stream fails while sending ipcData
     * @throws InterruptedException If we are unable to wait to send ipcData
     */
    public void write(String payload) throws IOException, InterruptedException {
        /* ADB needs null-terminated strings */
        write(payload.getBytes("UTF-8"), true);
    }
