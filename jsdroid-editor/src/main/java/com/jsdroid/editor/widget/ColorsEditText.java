 ...
 * 颜色编辑框
 */
public class ColorsEditText extends android.support.v7.widget.AppCompatEditText {
    ...
    private int defaultTextColor = Colors.defaultTextColor;
    private int lineNumberColor = Colors.lineNumberColor;
    private int selectLineStrokeColor = Colors.selectLineStrokeColor;
    private int lineNumberSplitColor = Colors.lineNumberSplitColor;
    private int lineNumberSplitWidth = Colors.lineNumberSplitWidth;
    private int cursorColor = Colors.cursorColor;
    private int selectBackgroundColor = Colors.selectBackgroundColor;
    ...
    public void setSelectLineStrokeColor(int selectLineStrokeColor) {
        this.selectLineStrokeColor = selectLineStrokeColor;
    }
    ...
    public ColorsEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    private void assumeLayout() {
        try {
            Method assumeLayout = TextView.class.getDeclaredMethod("assumeLayout");
    ...
    private void scrollToOffset(int offset) {
        ...
        if (layout != null) {
            //滑动到对应的位置
            float horizontal = getLayout().getSecondaryHorizontal(offset);
            float offsetx = horizontal - getScrollView().getScrollX();
            if (offsetx < 0 || offsetx > getWidth()) {
                getScrollView().setScrollX((int) horizontal);
            }
        }
    }
    ...
    private void init() {
        ...
        setPadding(0, DpiUtils.dip2px(getContext(), 2), DpiUtils.dip2px(getContext(), 8), DpiUtils.dip2px(getContext(), 24));
    }
     ...
     * @return
     */
    private Map<Integer, String> getLineNumbers() {
     ...
     * @param ret
     */
    private void getLineRangeForDraw(Rect rect, int[] ret) {
        Layout layout = getLayout();
     ...
     * @return [位置，偏移]
     */
    private float[] getLineFirstCharPosForDraw(float[] widths) {
    ...
    private Method getUpdatedHighlightPathMthod;
     ...
     */
    private Path getUpdatedHighlightPath() throws NoSuchMethodException,
    ...
    private Field mHighlightPaintField;
     ...
     */
    private Paint getHighlightPaint() throws IllegalAccessException,
     ...
     * @throws IllegalAccessException
     */
    private Object getEditor() throws NoSuchFieldException, IllegalAccessException {
...
//        horizontal = Math.max(0.5f, horizontal - 0.5f);
     ...
     * @throws NoSuchFieldException
     */
    private void drawCursorAndSelectPath(Canvas canvas) throws NoSuchMethodException,
    ...
    public void dumpColors(String info) {
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < codeColors.length; i++) {
            builder.append(Integer.toHexString(codeColors[i]));
            builder.append(",");
        }
        Log.d("colors", info + ": " + builder);
    }
...
//        dumpColors("drawText");
        ...
        for (int lineNum = firstLine; lineNum <= lastLine
            ...
            if (lineNum == selectLine) {
                paint.setColor(selectLineStrokeColor);
