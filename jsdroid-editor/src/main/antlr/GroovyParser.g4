...
qualifiedClassName
    :   qualifiedNameElements classTip[2]
    ;
...
@leftfactor { classifiedModifiers }
    :   classifiedModifiers[$t]
        ...
        |   typeNamePairs nls ASSIGN nls variableInitializer  //XXX=XXX
        )
    |
        classifiedModifiers[$t]?
        type variableDeclarators?
...
statement
    ...
    // ImportInfo statement.  Can be used in any scope.  Has "import x as y" also.
 ...
 *  To handle empty cases at the stop, we add switchLabel* to statement.
 */
...
postfixExpression
    :   pathExpression op=(INC | DEC)?  //dot可能是error
    ;
...
expression
    ...
    :   castParExpression castOperandExpression                                             #expressionCastExprAlt
    |   postfixExpression                                                                   #expressionPostfixExprAlt

    // ~(BNOT)/!(LNOT) (level 1)
    |   (BITNOT | NOT) nls expression                                                       #expressionUnaryNotExprAlt

    // math power operator (**) (level 2)
    |   left=expression op=POWER nls right=expression                                       #powerExprAlt

    // ++(prefix)/--(prefix)/+(unary)/-(unary) (level 3)
    |   op=(INC | DEC | ADD | SUB) expression                                               #expressionUnaryAddExprAlt
...
pathExpression returns [int t]
    :   primary (pathElement { $t = $pathElement.t; })* pathEnd?
    ;
...
pathElement returns [int t]
    :   nls
        ...
        { $t = 1; }
    |
        nls DOT nls NEW (constructorTip|creator[1])
        { $t = 6; }
    |   arguments
        { $t = 2; }
...
primary
    :
        ...
        classTip[2]  typeArguments?                                                         #identifierPrmrAlt
    |   literal                                                                             #literalPrmrAlt
    |   gstring                                                                             #gstringPrmrAlt
    |   NEW nls (constructorTip|creator[0])                                                 #newPrmrAlt
    |   THIS                                                                                #thisPrmrAlt
    |   SUPER                                                                               #superPrmrAlt
...
constructorTip //构造方法提示
    :   standardClassOrInterfaceType
;
classTip[int t] //类名提示
    :   { 0 == $t }? (Identifier|CapitalizedIdentifier)
    |   { 1 == $t }? (Identifier|CapitalizedIdentifier)
    |   { 2 == $t }? identifier
;
methodOrFieldTip //方法或者属性提示
    :   classTip[2]
;
variableTip //变量提示
    :   classTip[2]
