...
public class Dialogs {
    ...
    public static QMUIDialog showCopyDialog(@NotNull final Context context, String title, String content, final String copy) {
        ...
        if (copy != null) {
            builder.addAction("复制", new QMUIDialogAction.ActionListener() {
                ...
                public void onClick(QMUIDialog dialog, int index) {
                    dialog.dismiss();
                    ClipboardUtil.copy(dialog.getContext(), copy);
                    TipHelper.showInfo(dialog.getContext(), "复制成功！");
                }
            });
        }
    ...
    public static void showShareFileDialog(final Context context, String title, String content, final String file) {
        QMUIDialog.MessageDialogBuilder builder = new QMUIDialog.MessageDialogBuilder(context);
        builder.setTitle(title);
        builder.setMessage(content);
        if (file != null) {
            builder.addAction("分享", new QMUIDialogAction.ActionListener() {
                @Override
                public void onClick(QMUIDialog dialog, int index) {
                    dialog.dismiss();
                    Shares.shareFile(context, new File(file));
                }
            });
        }
        builder.addAction("关闭", new QMUIDialogAction.ActionListener() {
